...
GBA::PaletteBlock16 * GBA::Palette::GetPaletteBlock(vu16 * paletteLocation, u8 index)
{
	return index < c_BlockCount4bbp ? paletteLocation + index : nullptr;
}

void GBA::Palette::LoadSpritePalette(u8 blockIndex, const ColourPalette16 & palette)
{
	PaletteBlock16* block = GetSpritePaletteBlock(blockIndex);
	LoadPalette(block, palette);
}

void GBA::Palette::LoadSpritePalette(const ColourPalette256 & palette)
{
