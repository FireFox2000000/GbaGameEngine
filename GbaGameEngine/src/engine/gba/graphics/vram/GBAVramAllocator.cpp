...
constexpr int ScreenBlocksPerCharBlock = 
	ARRAY_SIZE(GBATEK::VramTileMode::UBackgroundMapsAndTiles::screenBaseBlocks) /
	ARRAY_SIZE(GBATEK::VramTileMode::UBackgroundMapsAndTiles::characterBaseBlocks);
...
namespace GBA
{
	...
	tTileId VramAllocator::AllocSpriteMem(const GBATEK::UPixelData* pixelMap, u32 pixelMapSize, u32 compressionFlags)
	{
	...
	VramAllocator::tScreenBaseBlockIndex VramAllocator::AllocBackgroundMem(u32 dataLengthAsU16, bool charBlockAligned)
	{
		...
		constexpr int screenBlockSize = sizeof(GBATEK::vram->videoMode2.backgroundMapsAndTiles.screenBaseBlocks[0]);
	...
	TileBlockGroups VramAllocator::AllocBackgroundTileSetMem(u32 tileSetLength)
	{
		...
		TileBlockGroups cbbIndex = static_cast<TileBlockGroups>(GBATEK::VramTileMode::ScreenBaseBlockToCharacterBaseBlock(tileSbbIndex));
		DEBUG_LOGFORMAT("Loaded bg tileset into slot %d", (int)cbbIndex);

		return cbbIndex;
	}

	void VramAllocator::LoadBackgroundTileSetMem(const GBATEK::UPixelData* tileset, u32 tileSetLength, TileBlockGroups cbbIndex)
	{
		auto* dest = GBATEK::vram->videoMode2.backgroundMapsAndTiles.characterBaseBlocks[cbbIndex];
		VramSafeMemCopy(dest->pixelData, tileset, tileSetLength);
	}

	void VramAllocator::SetBackgroundTileSetMem(const GBATEK::UPixelData value, u32 tileSetLength, TileBlockGroups cbbIndex)
	{
		auto* dest = GBATEK::vram->videoMode2.backgroundMapsAndTiles.characterBaseBlocks[cbbIndex];
	...
	void VramAllocator::LoadBackgroundTileMapMem(const GBATEK::BackgroundTilemapEntry* mapData, u32 mapDataLength, tScreenBaseBlockIndex sbbIndex)
	{
		auto* dest = GBATEK::vram->videoMode2.backgroundMapsAndTiles.screenBaseBlocks[sbbIndex];
		u32 byteLength = mapDataLength * sizeof(*mapData);
		VramSafeMemCopy(dest, mapData, byteLength);
	}

	void VramAllocator::SetBackgroundTileData(tScreenBaseBlockIndex sbbIndex, u32 offset, GBATEK::BackgroundTilemapEntry data)
	{
		GBATEK::vram->videoMode2.backgroundMapsAndTiles.screenBaseBlocks[sbbIndex][offset] = data;
	}

	void VramAllocator::SetBackgroundTileData(tScreenBaseBlockIndex sbbIndex, u32 offset, GBATEK::BackgroundTilemapEntry data, int dataCount)
	{
		auto* dest = &(GBATEK::vram->videoMode2.backgroundMapsAndTiles.screenBaseBlocks[sbbIndex][offset]);
		VramSafeMemSet(dest, data, dataCount);
	}

	void VramAllocator::SetBackgroundTileData(tScreenBaseBlockIndex sbbIndex, u32 offset, const GBATEK::BackgroundTilemapEntry* data, int dataSize)
	{
		auto* dest = &(GBATEK::vram->videoMode2.backgroundMapsAndTiles.screenBaseBlocks[sbbIndex][offset]);
	...
	void VramAllocator::FreeBackgroundTileSetMem(TileBlockGroups cbbIndex)
	{
		...
		u32 cbbSeIndex = GBATEK::VramTileMode::CharacterBaseBlockToScreenBaseBlock(cbbIndex);
	...
	bool VramAllocator::LoadTiles(const GBATEK::UPixelData* pixelMap, u32 pixelMapSize, u32 compressionFlags, TileBlockGroups tileBlockGroup, u16 startTileIndex)
	{
		GBATEK::BaseTile* tileBlock = nullptr;

		if (tileBlockGroup >= TileBlockGroups::SpriteLower)
		{
			tileBlock = GBATEK::vram->videoMode2.objectTiles;
		}
		else
		{
			tileBlock = GBATEK::vram->videoMode2.backgroundMapsAndTiles.characterBaseBlocks[tileBlockGroup];
		}

		// Allow tiles to bleed over into further char blocks
		GBATEK::UPixelData* tileMem = (tileBlock + startTileIndex)->pixelData;
		...
		if (compressionType == Compression::Type::BitPacked)
		{
			...
			auto& profilerClock = GBATEK::ioRegisterTimers->at(GBATimerId::Profile);
			profilerClock.frequency = GBATEK::ClockFrequency::Cycle_64;
		...
		else if ((u8*)(tileMem + pixelMapSize) <= ((u8*)(GBATEK::vram) + sizeof(GBATEK::vram->videoMode2)))
		{
#ifdef DECOMPRESSION_PROFILE
			auto& profilerClock = GBATEK::ioRegisterTimers->at(GBATimerId::Profile);
			profilerClock.frequency = GBATEK::ClockFrequency::Cycle_64;
